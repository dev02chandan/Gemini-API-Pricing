...
def calculate_cost(model, api_calls_per_day, avg_input_length, avg_output_length, image_count_per_day, video_seconds_per_day, audio_seconds_per_day, context_window):
    if model == 'Gemini 1.5 Flash':
        ...
        if context_window == '<= 128K':
            ...
            image_cost = 0.00263 * image_count_per_day
...
if st.sidebar.button("Calculate Cost"):
    daily_total_cost, monthly_total_cost, image_cost, video_cost, text_input_cost, audio_cost, text_output_cost = calculate_cost(
        model, api_calls_per_day, avg_input_length, avg_output_length, image_count_per_day, video_seconds_per_day, audio_seconds_per_day, context_window)
    ...
    st.write(f"- Total number of images processed per month: {image_count_per_day * 30}")
    st.write(f"- Cost per image: ${image_cost / image_count_per_day if image_count_per_day else 0:.6f}")
    ...
    st.write(f"- Cost per second of video: ${video_cost / video_seconds_per_day if video_seconds_per_day else 0:.6f}")
    ...
    st.write(f"- Cost per 1k characters: ${text_input_cost / (api_calls_per_day * avg_input_length / 1000) if (api_calls_per_day * avg_input_length / 1000) else 0:.6f}")
    ...
    st.write(f"- Cost per second of audio: ${audio_cost / audio_seconds_per_day if audio_seconds_per_day else 0:.6f}")
    ...
    st.write(f"- Cost per 1k characters: ${text_output_cost / (api_calls_per_day * avg_output_length / 1000) if (api_calls_per_day * avg_output_length / 1000) else 0:.6f}")
